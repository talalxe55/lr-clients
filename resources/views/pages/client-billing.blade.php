...
<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        <div class="container-fluid py-4">
            <form method="POST" action="{{route('create.invoice', ['clientid' => $client->id, 'billingid' => $billing->id])}}">
                @csrf
            <div class="page-content container">
                <div class="page-header text-blue-d2">
                    <h1 class="page-title text-secondary-d1">
                        Billing Details For {{date('M, Y', strtotime($billing->created_at))}}
                    ...
                    <div class="row mt-4">
                        <div class="col-12 col-lg-12">
                            ...
                            <div class="row">
                                ...
                                <div class="text-95 col-sm-6 align-self-start d-sm-flex justify-content-end">
                                    ...
                                    <div class="text-grey-m2">
                                        ...
                                        <div class="my-2"><i class="fa fa-circle text-blue-m2 text-xs mr-1"></i> <span class="text-600 text-90">ID:</span> #{{$billing->id}}</div>
            
                                        <div class="my-2"><i class="fa fa-circle text-blue-m2 text-xs mr-1"></i> <span class="text-600 text-90">Month:</span> <input type="date" name="month" readonly class="d-none form-control border border-0 p-2" value={{$billing->created_at}}> {{date('M, Y', strtotime($billing->created_at))}}</div>
            
                                        <div class="my-2"><i class="fa fa-circle text-blue-m2 text-xs mr-1"></i> <span class="text-600 text-90">Days Calculated:</span> <input type="number" name="days_count" readonly class="d-none form-control border border-0 p-2" value={{$billing->days_count}}> {{$billing->days_count}}</div>
                                    </div>
                                </div>
                                <!-- /.col -->
                            </div>
                            ...
                            <div class="mt-4">
                                ...
                                @php $counter = 0; $arraycounter = 0 @endphp
                                <div class="text-95 text-secondary-d3">
                                @foreach($services as $service)
                                <div class="row mb-2 mb-sm-0 py-25">
                                    <div class="d-none d-sm-block col-1">{{++$counter}} <input type="number" name="services[{{$arraycounter}}][service_id]" class="form-control border border-2 p-2 d-none" value={{$service->service_id}}></div>
                                    <div class="col-9 col-sm-5">
                                    <input type="text" name="services[{{$arraycounter}}][service_name]" class="form-control border border-0 p-2" readonly value={{$service->service_name}}>
                                    </div>
                                    <div class="d-none d-sm-block col-2">
                                    <input type="number" name="services[{{$arraycounter}}][service_quantity]" min="1" required class="form-control border border-2 p-2" value={{$service->service_quantity}}>
                                    @error("services[{{$counter}}]['service_quantity']")
                                    <p class='text-danger inputerror'>{{ $message }} </p>
                                    @enderror
                                    </div>
                                    <div class="d-none d-sm-block col-2 text-95">
                                    <input type="number" name="services[{{$arraycounter}}][service_amount]" step="any" min="1" required class="form-control border border-2 p-2" value={{is_float($service->service_amount)?number_format((float)$service->service_amount, 2, '.', ''):round($service->service_amount)}}>
                                    @error("services[{{$counter}}]['service_amount']")
                                    <p class='text-danger inputerror'>{{ $message }} </p>
                                    @enderror
                                    </div>
                                    <div class="col-2 text-secondary-d2">
                                    <input type="number" name="services[{{$arraycounter}}][service_total]" step="any" min="1" required class="form-control border border-2 p-2" value={{is_float($service->service_amount)?number_format((float)$service->service_amount, 2, '.', ''):round($service->service_amount)}}>
                                    @error("services[{{$counter}}]['service_total']")
                                    <p class='text-danger inputerror'>{{ $message }} </p>
                                    @enderror
                                    </div>
                                </div>
                                @php$arraycounter++@endphp
                                @endforeach
                                </div>
                                {{-- <div class="text-95 text-secondary-d3">
                                    <div class="row mb-2 mb-sm-0 py-25">
                        ...
                        -->
                                ...
                                <div class="row mt-3">
                                    <div class="col-12 col-sm-7 text-grey-d2 text-95 mt-2 mt-lg-0">
                                        ...
                                        <textarea id="w3review" name="billing_comments" class="form-control border border-2 p-2" rows="4" cols="50"></textarea>
                                        @error('billing_comments')
                                        <p class='text-danger inputerror'>{{ $message }} </p>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-12 col-sm-5 text-grey text-90 order-first order-sm-last">
                                        ...
                                        <div class="row my-2 align-items-center bgc-primary-l3 p-2">
                                            ...
                                            <div class="col-5">
                                                <input type="number" name="billing_total" min="1" required class="form-control border border-2 p-2" value={{$billing->total}}>
                                                @error('billing_total')
                                                <p class='text-danger inputerror'>{{ $message }} </p>
                                                @enderror
