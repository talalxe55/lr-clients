            ...
            <div class="card card-body mx-3 mx-md-4 mt-n6">
                ...
                <div class="card card-plain h-100">
                    ...
                    <div class="card-body p-3">
                        ...
                        @endif
                        <form method='POST' action='{{ route('update.service',['serviceid' => $service->id]) }}'>
                            @csrf
                            <div class="row">
                               
                                <div class="mb-3 col-md-6">
                                    <label class="form-label">Name</label>
                                    <input type="text" name="name" class="form-control border border-2 p-2" required value="{{$service->name}}">
                                    @error('name')
                                ...
                                <div class="mb-3 col-md-6">
                                    ...
                                    <input type="number" name="amount" class="form-control border border-2 p-2" value="{{$service->amount}}">
                                    @error('amount')
                                <p class='text-danger inputerror'>{{ $message }} </p>
                                @enderror
                                </div>
                            ...
                            <div class="row">
                                <div class="mb-3 col-md-6">
                                    ...
                                    <select class="text-capitalize form-select border border-2 p-2"
                                        placeholder="Amount Type" id="floatingTextarea2" name="amount_type"
                                        rows="4" cols="50">
                                        <option {{$service->amount_type==null||''?'selected':""}} selected disabled>Select Type</option>
                                        <option {{$service->amount_type=='fixed'?'selected':""}} class="text-capitalize" value="fixed">Fixed</option>
                                        <option {{$service->amount_type=='percentage'?'selected':""}} class="text-capitalize" value="percentage">Percentage</option>
                                    </select>
                                        @error('amount_type')
                                        <p class='text-danger inputerror'>{{ $message }} </p>
                                        @enderror
