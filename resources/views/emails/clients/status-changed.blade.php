...
<h2>Client {{$client->name}} status has been modified,</h2>
...
<div>
    ...
    @if($status)<p>Status Changed to: {{$status}}</p>@endif
    <p>Updated at: {{date('d-m-Y H:i:s',strtotime($client->updated_at))}} </p>
    <p>Updated By: {{$user->email}}</p>
